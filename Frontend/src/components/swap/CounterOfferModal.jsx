...
const CounterOfferModal = ({ isOpen, onClose, swap, onCounterOfferSubmit }) => {
  ...
  useEffect(() => {
    const fetchRequesterItems = async () => {
      ...
      try {
        setLoading(true)
        // Fetch the requester's items
        const response = await axios.get(`${API_URL}/api/users/${swap.requester._id}/items`, { withCredentials: true })

        // Filter out the item that's already being offered
        const items = response.data.items.filter((item) => item._id !== swap.offeredItem._id)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Error sending counter offer:", err)
      const errorMessage =
        err.response?.data?.message || `Failed to send counter offer. Status: ${err.response?.status || "unknown"}`
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Counter Offer">
      <div className="counter-offer-container">
        ...
        {loading ? (
          ...
          <div className="counter-offer-error">
            ...
            <button
              ...
              onClick={() => {
                setLoading(true)
                axios
                  .get(`${API_URL}/api/users/${swap.requester._id}/items`, { withCredentials: true })
                  .then((response) => {
                    const items = response.data.items.filter((item) => item._id !== swap.offeredItem._id)
                    setRequesterItems(items)
                    setError(null)
          ...
          <div className="counter-offer-items">
            {requesterItems.map((item) => (
              ...
              >
                <div className="counter-offer-item-image">
                  <img src={item.imageUrls[0] || "/placeholder.jpg"} alt={item.name} />
                </div>
                <div className="counter-offer-item-details">
                  <h3 className="counter-offer-item-name">{item.name}</h3>
                  <p className="counter-offer-item-meta">
        ...
        <div className="counter-offer-actions">
          ...
          <button className="submit-button" onClick={handleSubmit} disabled={!selectedItemId || submitting || loading}>
            {submitting ? "Sending..." : "Send Counter Offer"}
          </button>
        </div>
      </div>
