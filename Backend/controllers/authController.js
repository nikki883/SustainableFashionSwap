...
export const resendOtp = async (req, res) => {
  ...
  // Send OTP to the email (you can use the same email sending logic here)
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.GMAIL_USER, 
...
export const registerUser = async (req, res) => {
  try {
    ...
    const newUser = new User({
      ...
      email,
      password, 
      isVerified: true, // Set user as verified, assuming email verification is successful
    });
...
export const loginUser = async (req, res) => {
  ...
  console.log("Received login details:", { email, password });  // Add this for debugging
  ...
  try {
    ...
    // For debugging only - check if password looks hashed
    console.log("Password format check:", user.password.substring(0, 10) + "...");
    ...
    res.cookie("token", token, {
      ...
      sameSite: "Lax",  // or "Strict" if you need more security
    ...
    console.error("Error during login:", err);  // Detailed error logging
...
export const deleteUser = async (req, res) => {
  ...
  try {
    ...
    res.clearCookie("token", {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production", // This makes it work only in HTTPS
      sameSite: "Lax", // Or "Strict" for better security
      path: "/", // Ensure this path matches your app's structure
    });
...
export const changePassword = async (req, res) => {
  ...
  try {
    // Step 3: Find the user from the database
    const user = await User.findById(req.user._id);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }

    // Step 4: Compare current password with the stored password
